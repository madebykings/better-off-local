-- 084_loyalty_rpcs_and_grants.sql
-- RPCs for the loyalty stamp-card system.
--
-- process_loyalty_stamp
--   Called by the validate-qr-token edge function when a loyalty_visits offer
--   QR is scanned.  Mirrors the 12-step pattern of redeem_offer_token:
--     0.  Idempotency check
...
--     7.  Offer live, correct type, correct retailer, in date window
--     8.  Load or create loyalty card (SELECT … FOR UPDATE on card row)
--     9.  Card not already claimed or expired
--    10.  If card completed and presenting again → mark claimed, record redemption
--    11.  Cooldown check (min_hours_between_stamps)
--    12.  Add stamp row, update stamps_earned
--    13.  If stamps_earned = stamps_required → mark card completed
--    14.  Consume token, insert redemption, record idempotency attempt
--    15.  Return result
--
-- get_loyalty_card_state
--   Lightweight read RPC for the mobile app to fetch card progress.
...
-- ── get_loyalty_card_state ────────────────────────────────────────────────────
-- Lightweight read for the mobile app.  Returns current card state for a given
-- profile+offer pair.  Returns zero rows when no card has been created yet.

create or replace function get_loyalty_card_state(
  p_profile_id uuid,
  p_offer_id   uuid
)
returns table(
...
as $$
  ...
  where lc.profile_id = p_profile_id
    and lc.offer_id   = p_offer_id
  limit 1;
$$;

-- Callable by authenticated users (members read their own card).
-- RLS on loyalty_cards already enforces ownership; this RPC is an optimisation.
grant execute on function get_loyalty_card_state(uuid, uuid) to authenticated;

-- ── Admin/service helpers ─────────────────────────────────────────────────────

-- Expire loyalty cards for offers whose end_at has passed.
-- Intended to be run daily by a cron job or Supabase scheduled function.
create or replace function expire_overdue_loyalty_cards()
returns integer
language plpgsql
security definer
set search_path = public, pg_temp
...
revoke execute on function expire_overdue_loyalty_cards() from public;
revoke execute on function expire_overdue_loyalty_cards() from anon;
revoke execute on function expire_overdue_loyalty_cards() from authenticated;
grant  execute on function expire_overdue_loyalty_cards() to service_role;
