...
export const metadata: Metadata = { title: 'Referral Campaign – Retailer Portal' };
...
export default async function ReferralCampaignDetailPage({ params }: Props) {
  const { offerId } = await params;
  const { retailerId } = await requireRetailerUser();
  const supabase = createServiceClient();

  const [offerResult, rulesResult, locationsResult, offerLocationsResult, loyaltyConfigResult, referralConfigResult] = await Promise.all([
    supabase
      .from('offers')
    ...
    supabase
      ...
      .select('reward_title, reward_description, friend_reward_enabled, friend_reward_title, friend_reward_description, max_rewards_per_referrer')
      .eq('offer_id', offerId)
      .maybeSingle(),
  ]);
  ...
  const referralCfgRow = referralConfigResult.data;
  const venueReferralConfig: VenueReferralConfigFields | undefined = referralCfgRow
    ? {
        rewardTitle: referralCfgRow.reward_title ?? '',
        rewardDescription: referralCfgRow.reward_description ?? '',
        friendRewardEnabled: referralCfgRow.friend_reward_enabled ?? false,
        friendRewardTitle: referralCfgRow.friend_reward_title ?? '',
        friendRewardDescription: referralCfgRow.friend_reward_description ?? '',
        maxRewardsPerReferrer: referralCfgRow.max_rewards_per_referrer != null
          ? String(referralCfgRow.max_rewards_per_referrer)
          : '',
      }
    : offerType === 'venue_referral'
    ? EMPTY_VENUE_REFERRAL_CONFIG
    : undefined;
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Referral campaign</p>
      </div>
      <GuidanceCard
        heading="About referral campaigns"
        body="Your referral campaign lets members share a personal link with friends. When a friend visits and redeems an offer, the referrer earns a reward."
      />
      <OfferForm
        mode="edit"
        offerId={offer.id}
