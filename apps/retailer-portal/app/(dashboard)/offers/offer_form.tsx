...
export function OfferForm({ mode, offerId, offerStatus, initialData, locations }: OfferFormProps) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {mode === 'edit' && !canEdit && (
        <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500 mb-4">
          This offer cannot be edited in its current status ({offerStatus}).
        </div>
      )}

      {/* Tab bar */}
      <div className="flex overflow-x-auto border-b border-gray-200 mb-6 -mx-0 scrollbar-none">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
            ...
            className={[
              'flex shrink-0 items-center px-4 py-3 text-sm font-medium whitespace-nowrap',
      ...
      <form id="offer-form" onSubmit={handleSubmit} noValidate>
        ...
        {/* Basics tab */}
        <div className={activeTab === 'basics' ? 'space-y-6 py-6' : 'hidden'}>
        ...
        {/* Deal Details tab */}
        <div className={activeTab === 'details' ? 'space-y-6 py-6' : 'hidden'}>
          {!fields.offerType && (
            <GuidanceCard
              heading="Select an offer type first"
              body="Select an offer type on the Basics tab first."
            />
          )}
        ...
        {/* Rules tab */}
        <div className={activeTab === 'rules' ? 'space-y-6 py-6' : 'hidden'}>
        ...
        {/* Image tab */}
        <div className={activeTab === 'image' ? 'space-y-6 py-6' : 'hidden'}>
          <Field
            label="Cover image"
      ...
      {/* Preview tab (outside form, no form submission needed) */}
      <div className={activeTab === 'preview' ? 'py-4' : 'hidden'}>
        <div className="py-4 space-y-4">
          <p className="text-sm text-gray-500">This is how your offer appears to members in the app.</p>

          <div className="max-w-sm">
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              {fields.imageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={fields.imageUrl} alt="" className="w-full h-32 object-cover" />
              ) : (
                <div className="w-full h-32 bg-gradient-to-r from-green-700 to-green-500 flex items-center justify-center">
              ...
              <div className="p-3">
                {fields.offerType && (
                  <span className={[
                    'inline-block rounded px-2 py-0.5 text-xs font-bold text-white mb-2',
                    fields.offerType === 'percentage_discount' ? 'bg-green-700' :
                    fields.offerType === 'fixed_discount' ? 'bg-blue-600' :
                    fields.offerType === 'loyalty_visits' ? 'bg-teal-600' :
                    fields.offerType === 'venue_referral' ? 'bg-amber-600' :
                    ...
                    {computeValueText(fields.offerType, fields.discountValue) || OFFER_TYPE_CONFIG[fields.offerType]?.label}
                  </span>
                )}

                <p className="font-semibold text-gray-900 text-sm">{fields.headline || 'Your offer headline'}</p>
                <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{fields.description || 'Your offer description will appear here.'}</p>

                {fields.estimatedSaving && (
                  <p className="text-xs text-green-700 font-medium mt-2">Save ~£{fields.estimatedSaving}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <p className="text-xs text-gray-400">
            The actual appearance may vary slightly. Fill in your Basics and Deal Details to improve this preview.
          </p>
        </div>
      </div>

      {/* Review tab (outside form — uses separate server actions) */}
      <div className={activeTab === 'review' ? 'py-4' : 'hidden'}>
        <div className="py-4 space-y-4">
          <div className={`rounded-lg border px-4 py-3 ${
            !offerStatus || offerStatus === 'draft' ? 'bg-gray-50 border-gray-200' :
            offerStatus === 'pending' ? 'bg-amber-50 border-amber-200' :
            offerStatus === 'live' ? 'bg-green-50 border-green-200' :
            offerStatus === 'rejected' ? 'bg-red-50 border-red-200' :
            'bg-gray-50 border-gray-200'
          }`}>
            <p className="text-sm font-semibold text-gray-800 capitalize">
              {offerStatus ? offerStatus.replace('_', ' ') : 'Draft'}
            </p>
            <p className="text-sm text-gray-600 mt-0.5">
              {!offerStatus || offerStatus === 'draft'
                ? 'Save your offer on the Basics tab, then submit for review when ready.'
                : offerStatus === 'pending'
                ? 'Your offer is being reviewed. This usually takes 1–2 business days.'
                : offerStatus === 'live'
                ? 'Your offer is live and visible to members.'
                : offerStatus === 'paused'
                ? 'Your offer is paused and not visible to members.'
                : offerStatus === 'rejected'
                ? 'Your offer needs changes. Update your details and save to resubmit.'
                : ''}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {isLiveOrPaused && (
              <form action={togglePauseOffer}>
                <input type="hidden" name="offer_id" value={offerId} />
                <button type="submit" className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">
            ...
            {isDraft && offerId && (
              <form action={deleteDraftOffer}>
                ...
                <button
                  type="submit"
                  onClick={(e) => { if (!confirm('Delete this draft offer?')) e.preventDefault(); }}
            ...
            {isLiveOrPaused && (
              <form action={archiveOffer}>
                ...
                <button
                  type="submit"
                  onClick={(e) => { if (!confirm('Archive this offer? It will no longer be visible to members.')) e.preventDefault(); }}
            ...
            {mode === 'create' && (
              <p className="text-sm text-gray-400">Save the offer first, then come back here to submit it for review.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {canEdit && activeTab !== 'review' && (
        <StickyActionBar>
          <button
            ...
            form="offer-form"
            disabled={isPending}
            className="rounded-lg bg-green-800 px-6 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isPending
