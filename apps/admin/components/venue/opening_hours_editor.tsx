...
const inputCls =
  'rounded-lg border border-gray-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-green-700';

interface Props {
  locationId: string;
  initialData: OpeningHoursData;
  saveAction: (locationId: string, hours: OpeningHoursData) => Promise<{ error?: string } | null>;
}

export function AdminOpeningHoursEditor({ locationId, initialData, saveAction }: Props) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveAction(locationId, hours);
      if (!result) {
        setSaved(true);
      } else {
